...
function SortFilter() {
  ...
  const inputChosenOptionHandler = (e, index, filter) => {
    ...
    if (
      ...
      inputChecked.current[index].current.checked === true
    ) {
      return dispatch({ type: 'CHOSEN_OPTION_BRAND', filter: e.target.value });
    } else if (
      e.target.nodeName === 'INPUT' &&
      inputChecked.current[index].current.checked === false
    ) {
        ...
        <div className="wrapper">
          <div className="chosen__options">
            {uniqueName.map((filter, index) => (
              <div
                onClick={() =>
                  dispatch({ type: 'REMOVE_CHOSEN_OPTION', filter })
                }
          ...
          <div className="filter">
            ...
            <div className="gender filter__options">
              <div
                ...
                onClick={() =>
                  setProductNameElm({
                    ...productNameElm,
                    state: !productNameElm.state,
                  })
                }
              >
                <h3>product name</h3>
                <Add
                  className={`${productNameElm.state && 'show__add__icon'}`}
                />
                <Remove
                  className={`remove__icon ${
                    productNameElm.state && 'show__remove__icon'
                  }`}
                />
              </div>
            ...
            <div className="brand filter__options">
              <div className="title" onClick={() => setBrandEle(!brandEle)}>
                ...
                <Remove
                  className={`remove__icon ${brandEle && 'show__remove__icon'}`}
                />
              </div>
                ...
                <form>
                  {productTitle.productBrand.map((data, index) => (
                    <div className="input" key={index}>
                      <input
                        onClick={(e) =>
                          inputChosenOptionHandler(e, index, data)
                        }
                        type="checkbox"
                        ref={inputChecked.current[index]}
                        name={data}
                        value={data}
                      />
                      <label className="label" htmlFor="adidas">
                        {' '}
                        {data}{' '}
                      </label>
                      <span> (298)</span>
